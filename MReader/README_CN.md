# Hawk’s MReader 组件

**MReader** (Markdown Reader) 是一个轻量级工具，旨在将 Markdown 文件转换为独立、美观的 HTML 阅读页面。它的设计初衷是模仿“微信公众号”的视觉风格，并支持 GitHub Alerts 和 Ruby 拼音标注等高级特性。

## 📂 功能特性

*   **完全独立**：生成包含所有 CSS 和脚本的单个 `.html` 文件。无需服务器即可直接打开。
*   **微信风格**：内置模仿微信文章的默认样式，阅读体验亲切。
*   **空格键自动滚动**：支持提词器模式，方便沉浸式阅读或演讲。
*   **高级 Markdown 支持**：
    *   GitHub 警示框 (Alerts)：支持 `[!NOTE]`, `[!TIP]`, `[!WARNING]` 等语法。
    *   Ruby 拼音标注：`[汉]{py}` -> `<ruby>汉<rt>py</rt></ruby>`。

## 🚀 快速上手

### 1. 生成阅读页面

使用该目录下的 Python 脚本进行转换：

```bash
# 基本用法 (使用默认样式 default.css)
python mreader.py /path/to/your/article.md

# 指定输出路径
python mreader.py /path/to/article.md -o /path/to/site/article.html

# 使用自定义 CSS 样式
python mreader.py /path/to/article.md /path/to/custom_style.css
```

### 2. 集成到网页中

生成的 HTML 是自包含的。若要将其集成到其他网页（如课程平台或博客）且不产生样式冲突，建议使用 `<iframe>`。

**示例代码：**

```html
<!-- 使用 IFrame 进行集成 -->
<div class="reader-wrapper" style="width: 100%; max-width: 500px; margin: 0 auto; height: 80vh; border: 1px solid #ddd; border-radius: 20px; overflow: hidden;">
    <iframe 
        src="./path/to/generated_article.html" 
        style="width: 100%; height: 100%; border: none;"
        title="Hawk Reader"
    ></iframe>
</div>
```

## 🛠 文件结构

*   `mreader.py`: 核心转换脚本。
*   `template.html`: HTML 模板骨架。
*   `default.css`: 内置样式文件（微信公众号视觉风格）。
