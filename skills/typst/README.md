...
- **模式 B：移动端自适应无缝长图模式 (`long`)**
  - **100% 遵循出版级排版规范**：正文字体字阶（思源宋体 10.5pt）、中文段首缩进（严格缩进两字符 2em）、两端对齐（`justify: true`）、列表排版、科技三线表与 LaTeX 公式保护系统与教材版完全一致，保持极致的学术严谨美感；
  - **仅去除页眉与页脚**：显式设置 `header: none, footer: none`，消除物理页面切分导致的受控页眉与分页页码干扰；
  - **一镜到底连续版心**：`height: auto` 彻底消除跨页断层，三线表、矢量架构图与长文一镜到底无缝浏览；
  - **超清光栅化位图输出**：支持 `--ppi 200/300`，一键生成无损高清 PNG，适合手机长图阅读与群聊技术沟通。
