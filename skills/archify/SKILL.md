---
name: archify
description: Create polished, validated architecture, workflow, sequence, data-flow, and lifecycle/state diagrams as explorable standalone HTML with inline SVG, dark/light themes, optional trace motion, and publication-grade clean SVG export for Typst/LaTeX/print with automated typography scaling, arrow boundary snapping, and translucent badge styling. Accept plain-language requirements or pasted Mermaid flowchart, sequenceDiagram, and stateDiagram input; inspect repository evidence when the diagram must reflect real code. Use when the user asks to visualize system architecture, infrastructure, cloud/security/network topology, technical workflows, API call sequences, request lifecycles, data pipelines, ETL/ELT, data lineage, state machines, or to convert/beautify Mermaid.
license: MIT
metadata:
  version: "2.18"
  author: tt-a1i
...
- **Installation modes**: Supports project-local installation (`.agents/skills/archify`) and global symlink references (e.g. `~/.agent/skills/archify` linked from a central repository).
- **Symlink & sandbox contract**:
  - When invoked from a workspace different from the physical installation directory, host IDE file-viewing tools may restrict cross-workspace traversal. The agent **MUST NOT** degrade to manual unvalidated approximations or hand-written templates.
  - The agent **MUST** dynamically resolve the physical skill directory (via `readlink`, realpath, or shell inspection) and execute the packaged CLI tools directly with Node.js (`node bin/archify.mjs ...`).
  - If a command operates outside the active workspace, execute via the native shell environment where the symlinked package resides or confirm with the user.
...
- Omit `meta.legend` for the truthful `auto` default. When needed, configure `title` (bounded 1-80 character custom legend heading overriding "Legend" / "图例"), `mode: auto|all|hidden`, and renderer-supported `entries.<kind>.label|visible`; labels and titles never change semantics.
...
## Publication-grade clean SVG export (Typst / LaTeX / PDF)

When the user asks to export or embed Archify diagrams into paper textbooks, technical specifications, or academic manuscripts (Typst, LaTeX, or standalone SVG), use the headless clean export tool to transform interactive HTML into publication-grade vector graphics:

```bash
# Export single HTML diagram to clean SVG
node scripts/export-clean-svg.mjs <input.html> <output.clean.svg> [light|dark]

# Batch export all HTML diagrams in docs/figures/
node scripts/export-clean-svg.mjs
```

### Publication invariants

1. **Stripped Web Chrome**: Completely remove UI overlays, search bars, zoom toolbars, guided tour bubbles, story overlays, and accessibility probe layers.
2. **Computed Style Inlining**: Convert all CSS variables (`var(--...)`) into permanent static XML attributes (`fill`, `stroke`, `font-family`, `font-size`) so external renderers (Typst/rsvg/Inkscape) render identically.
3. **Typography Baselines**:
   - Node titles: $\ge 17.5\text{px}$ (weight: 700, high-contrast dark `#0f172a`)
   - Context / Sub-labels: $\ge 14.5\text{px}$ (weight: 600, contrast slate `#334155`)
   - Fine tags: $\ge 13.5\text{px}$ (weight: 700)
   - Boundary / Stage titles: $\ge 16.0\text{px}$ (weight: 800)
4. **Frosted Translucent Badges (同色系毛玻璃徽章)**: Replace raw opaque white masks (`#ffffff`) with $14\%\sim 22\%$ low-opacity translucent fills matching the node/boundary color family, preventing harsh boundary cutouts while letting underlying structural grids show softly through.
5. **Arrow Boundary Snapping (箭头吸附外沿防吞没)**: All path endpoints must terminate exactly on the outer boundary of destination nodes (e.g. $X = \text{node.x}$ for leftward entry), never penetrating into node interiors where cards overlay and swallow arrowheads.
6. **Adaptive Label Box Width**: Label rectangles must guarantee at least $15\sim 20\text{px}$ horizontal breathing padding relative to text length to completely eliminate text overflow.
7. **ViewBox Normalization & Boundary Closure**: Strip excess bottom dead whitespace and ensure lane boxes (`c-lane`) maintain $\ge 20\text{px}$ breathing margins within the viewBox so bottom dashed lines and rounded corners close 100%.

For complete architectural math and Typst integration examples, see `references/publication-clean-svg.md`.
